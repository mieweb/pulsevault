...
async function main() {
  ...
  if (!process.env.DATABASE_URL) {
    ...
    assert.equal(
      dbUp.status,
      0,
      'docker compose up -d db should succeed (Docker is required for e2e)',
    );
  }
  ...
  // must exit non-zero instead of falling back to an open server.
  {
    ...
    const noSecret = spawn(process.execPath, ['server.mjs'], {
      cwd: demoDir,
      env,
      stdio: 'ignore',
    });
  ...
  try {
    ...
    console.log(
      `  ✓ /capabilities reports protocolVersion=${caps.protocolVersion}, uploadUnit=${caps.uploadUnit}`,
    );
    ...
    console.log(
      '  ✓ dashboard: /deeplinks without a session -> 401; Better Auth sign-in issues one',
    );
    ...
    assert.equal(
      ...
      half,
      'HEAD should report the offset after chunk 1',
    );
