...
test("web: Range requests — 206 bounded/suffix/open, 416 unsatisfiable, HEAD headers", async () => {
  ...
  try {
    ...
    const range = (header) =>
      ctx.vault.handler(new Request(url, { headers: { range: header } }));
    ...
    assert.equal(
      Buffer.compare(Buffer.from(await suffix.arrayBuffer()), body.subarray(900)),
      0,
    );
...
test("web: capabilities payload — no directUpload for local storage", async () => {
  const ctx = await startWebApp();
  try {
    const res = await ctx.vault.handler(
      new Request(`${BASE}${PREFIX}/capabilities`),
    );
