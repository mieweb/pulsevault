  ...
  try {
    ...
    for (const call of patchCalls) {
      assert.equal(
        call.artifactId,
        ID1,
        'authorize must see the real artifactId, not the kind prefix or undefined',
      );
    }
  } finally {
    await ctx.teardown();
...
test('PATCH/HEAD with an unresolvable upload id is a hard 403 when authorize is configured (PROTOCOL.md §5.2)', async () => {
  ...
  try {
    ...
    assert.equal(
      patch.status,
      403,
      'unresolvable PATCH target must be rejected, not silently allowed',
    );
...
test('a crafted multi-segment PATCH URL cannot smuggle bytes into a different artifact than authorize() checked', async () => {
  ...
  try {
    ...
    const victimCreate = await tusCreate(ctx.baseUrl, {
      artifactId: ID2,
      filename: 'clip.mp4',
      size: 1024,
    });
    const attackerCreate = await tusCreate(ctx.baseUrl, {
      artifactId: ID1,
      filename: 'clip.mp4',
      size: 1024,
    });
    ...
    assert.equal(
      patch.status,
      403,
      "must be rejected, not silently write to the victim's artifact",
    );

    const patchCall = authorizeCalls.find((c) => c.phase === 'patch');
    assert.equal(
      patchCall.artifactId,
      ID2,
      'authorize must see the artifactId @tus/server will actually operate on',
    );

    const victimBytes = await fs
      .readFile(path.join(ctx.workspaceDir, 'video', `${ID2}.mp4`))
      .catch(() => null);
    assert.equal(
      victimBytes.length,
      0,
      "victim's file must be untouched (still 0 bytes from create)",
    );
  } finally {
    await ctx.teardown();
  }
...
test('createCapabilityAuthorize: PATCH without a valid token is rejected, not silently accepted', async () => {
  ...
  try {
    ...
    assert.equal(
      wrongToken.status,
      403,
      'PATCH with a token for a different artifact must be rejected',
    );
...
test('createCapabilityAuthorize: scoped tokens — a resolve-only (watch) token cannot create or delete', async () => {
  const secret = 'test-secret';
  const issuer = 'https://vault.example.test';
  const lookupSecret = (kid) => (kid === 'k1' ? secret : null);
  const ctx = await startApp({
  ...
  try {
    // Upload with an upload-scoped token (create + patch only).
    const uploadToken = issueCapabilityToken(ID1, secret, {
      keyId: 'k1',
      issuer,
      scope: ['create', 'patch'],
    });
    ...
    // The upload token does NOT authorize playback (resolve) or delete.
    const getWithUploadToken = await fetch(artifactUrl(ctx, ID1), {
      redirect: 'manual',
      headers: { Authorization: `Bearer ${uploadToken}` },
    });
    assert.equal(getWithUploadToken.status, 403, 'upload-scoped token must not resolve');

    // A watch token (resolve only) plays back…
    const watchToken = issueCapabilityToken(ID1, secret, {
      keyId: 'k1',
      issuer,
      scope: ['resolve'],
    });
    ...
    assert.equal(get.status, 200, 'watch token resolves');

    // …but cannot delete the artifact or create new ones in the session —
    // the leak scenario scope exists to close (watch URL in logs/history).
    const del = await fetch(artifactUrl(ctx, ID1), {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${watchToken}` },
    });
    assert.equal(del.status, 403, 'watch token must not delete');
    ...
    assert.equal(create.status, 403, 'watch token must not create related artifacts');

    // An unscoped token still authorizes every phase (pre-scope back-compat).
    const unscoped = issueCapabilityToken(ID1, secret, { keyId: 'k1', issuer });
    const delUnscoped = await fetch(artifactUrl(ctx, ID1), {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${unscoped}` },
    });
    assert.equal(delUnscoped.status, 204, 'unscoped token keeps full capability');
  } finally {
    await ctx.teardown();
  }
});
